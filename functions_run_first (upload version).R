#this is the function for method used in real data analysis and simulation result#
...
library(matlib)
library(MASS)
quadratic_test<-function(z_vector, estimated_cov){
  chi_stat<-t(z_vector)%*%ginv(estimated_cov)%*%z_vector
  p_value<-pchisq(chi_stat,length(z_vector),lower.tail = F)
...
true_cov_mat_generation_autoregressive<-function(m,rho){
...
metaCCA <- function(est_genetic_cov,est_pheno_cov,est_XY_cov,N){
...
standard_chisq<-function(z_vector,correlation_matrix){
  chi_stat<-t(z_vector)%*%ginv(correlation_matrix)%*%z_vector
  pvalue<-pchisq(chi_stat,length(z_vector),lower.tail = F)
  pvalue<-as.numeric(pvalue)
  return(pvalue)
}

calculate_stat_T3<-function(z_vector,correlation_matrix,c_fixed){
  index_c<-ceiling(length(z_vector)*c_fixed)
  z_vector_order<-order(abs(z_vector),decreasing = T)
  ...
  stat<-t(select_vector)%*%ginv(select_correlation_matrix)%*%select_vector
...
#######Please see Setp 2 in our mauscript for more information
#######m stantds for the dimension, n is the number of sample size
generate_null_distribution_T3<-function(m,n,correlation_matrix,cutoff_value){
  true_mean<-rep(0,m)
  bootstrap_data<-mvrnorm(n,true_mean,correlation_matrix)
...
T_3<-function(z_vector,correlation_matrix,cutoff_value,coefficient_matrix){
...
new_method<-function(z_mat, est_genetic_cor, est_pheno_cor, cutoff_value, coefficient_matrix){
  z_mat<-t(z_mat)
  ...
  est_total_cov_mat<-kronecker(est_genetic_cor,est_pheno_cor)
  ...
  return(list(p_value_final=p_value_final, p_3=p_3,p_1=p_genetic_structure,p_2=p_pleiotropic))
}
################################################################################